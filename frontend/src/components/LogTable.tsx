...
useEffect(() => {
  ...
  const loadData = useCallback(async (page: number, append: boolean = false, sourceOverride?: ViewSource) => {
    ...
    try {
      ...
      const newLogs = Array.isArray(data) ? data : (data.logs || data.rows || []);
  ...
  Application Logs{" "}
  {isLive && (
    <span className="text-green-500 animate-pulse text-sm ml-2">
      ● Live
    </span>
  )}
</CardTitle>
  ...
  <div className="bg-red-500 text-white p-2 rounded mb-2">
    🚨 Critical errors detected in system!
  </div>
)}
...
>
                  ...
                  <TableCell className="whitespace-normal break-words font-mono text-sm">{log.message}</TableCell>
